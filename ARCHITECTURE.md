...
- **Media preview/source quality:** Coordinator `POST /api/v1/download/preview` dispatches transient `resolve_download` work (`operation: "preview"`) through Python `SourceRouter` to `services/youtube/`. It returns normalized title/thumbnail/uploader and descending available source resolutions without creating a download. Confirmation uses existing `POST /api/v1/download` with `quality`; Python resolves that source rendition and optional audio. Storage routes YouTube downloads to a dedicated flow under `backend/storage/media_download/youtube/` (separate from archive processing), downloading resolved streams, muxing with ffmpeg when adaptive, inspecting compatibility, performing conversion via `.convert-video/` inside the job workspace when required, and committing directly to the target destination without an intermediate title-named directory. See `backend/coordinator/docs/PROTOCOL.md` for cancellation and payloads. Home archive and media actions open separate forms in both clients.
