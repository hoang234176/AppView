...
// Job represents the isolated lifecycle of a YouTube media download task in Storage.
type Job struct {
	ID                    string
	CanonicalID           string
	URL                   string
...
// Snapshot is the thread-safe, serializable projection of a YouTube Job.
...
// StateDir returns the directory where YouTube job JSON files are persisted:
// APPVIEW_STATE_DIR/media_download/youtube/jobs
func StateDir() (string, error) {
	root, err := baseDir()
	if err != nil {
		return "", fmt.Errorf("không xác định được thư mục trạng thái AppView: %w", err)
	}
	return filepath.Join(root, "media_download", "youtube", "jobs"), nil
}

// WorkspaceDir returns the private workspace directory for a specific YouTube job:
// APPVIEW_STATE_DIR/media_download/youtube/workspaces/<job-id>
func WorkspaceDir(id string) (string, error) {
	...
	return filepath.Join(root, "media_download", "youtube", "workspaces", clean), nil
}

// StatePath returns the file path for a specific YouTube job JSON:
// APPVIEW_STATE_DIR/media_download/youtube/jobs/<job-id>.json
