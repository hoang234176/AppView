...
// commitInstagramMedia safely moves downloaded items to destination and emits folder invalidations.
func commitInstagramMedia(ctx context.Context, job *Job, workspace string) error {
	job.mu.RLock()
	...
	if len(items) == 0 {
		...
		_, err := commitMediaFile(ctx, sourceFile, destination, filename, jobID)
		return err
	}

	// Multiple items (photos/videos/mixed): copy each media file directly into destinationPath without creating a subfolder
...
// commitMediaFile safely copies a media file directly to destination without creating a subfolder.
