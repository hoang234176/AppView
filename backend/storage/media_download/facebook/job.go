...
// DownloadItem represents an individual media item (e.g. photo) in a post.
...
// Job represents the isolated lifecycle of a Facebook media download task in Storage.
type Job struct {
	ID                    string
	CanonicalID           string
	URL                   string
...
// Snapshot is the thread-safe, serializable projection of a Facebook Job.
...
// StateDir returns the directory where Facebook job JSON files are persisted:
// APPVIEW_STATE_DIR/media_download/facebook/jobs
func StateDir() (string, error) {
	root, err := baseDir()
	if err != nil {
		return "", fmt.Errorf("không xác định được thư mục trạng thái AppView: %w", err)
	}
	dir := filepath.Join(root, "media_download", "facebook", "jobs")
...
// WorkspaceDir returns the private workspace directory for a specific Facebook job:
// APPVIEW_STATE_DIR/media_download/facebook/workspaces/<job-id>
func WorkspaceDir(id string) (string, error) {
	...
	return filepath.Join(root, "media_download", "facebook", "workspaces", clean), nil
}

// StatePath returns the file path for a specific Facebook job JSON:
// APPVIEW_STATE_DIR/media_download/facebook/jobs/<job-id>.json
