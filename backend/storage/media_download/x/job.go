...
// DownloadItem represents an individual media item (e.g. photo, video, or post text) in an X post.
...
// Job represents the isolated lifecycle of an X media download task in Storage.
type Job struct {
	ID                    string
	CanonicalID           string
	URL                   string
...
// Snapshot is the thread-safe, serializable projection of an X Job.
...
// StateDir returns the directory where X job JSON files are persisted:
// APPVIEW_STATE_DIR/media_download/x/jobs
func StateDir() (string, error) {
	root, err := baseDir()
	if err != nil {
		return "", fmt.Errorf("không xác định được thư mục trạng thái AppView: %w", err)
	}
	dir := filepath.Join(root, "media_download", "x", "jobs")
...
// WorkspaceDir returns the private workspace directory for a specific X job:
// APPVIEW_STATE_DIR/media_download/x/workspaces/<job-id>
func WorkspaceDir(id string) (string, error) {
	...
	return filepath.Join(root, "media_download", "x", "workspaces", clean), nil
}

// StatePath returns the file path for a specific X job JSON:
// APPVIEW_STATE_DIR/media_download/x/jobs/<job-id>.json
