...
func TestVideoPipelineRegressions_Cases1to7(t *testing.T) {
	...
	// Case 3: original-video/ excluded from recursive discovery
	tempDir := t.TempDir()
	origDir := filepath.Join(tempDir, "original-video")
	subOrigDir := filepath.Join(origDir, "nested")
	if err := os.MkdirAll(subOrigDir, 0755); err != nil {
		t.Fatal(err)
	}
	// Write dummy video files inside original-video/
	if err := os.WriteFile(filepath.Join(origDir, "source1.mp4"), []byte("dummy-mp4-data"), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(subOrigDir, "source2.mp4"), []byte("dummy-mp4-data"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range validations {
		if strings.Contains(filepath.ToSlash(v.Path), "/original-video/") {
			t.Fatalf("Case 3: original-video/ must be excluded from recursive discovery, found: %s", v.Path)
		}
	}
...
func TestVideoPipelineRegressions_Cases8to12_DecisionsAndApply(t *testing.T) {
	...
	// Case 12: duplicate Apply cannot start duplicate conversion
	if err := ApplyVideoDecisions(job.ID, nil); err == nil {
...
func TestVideoPipelineRegressions_Cases13to18_CancellationLifecycle(t *testing.T) {
	...
	// Case 17: cancelled only after successful finalization
	job.mu.RLock()
	finalStage := job.Stage
	job.mu.RUnlock()
	if finalStage != "cancelled" {
		t.Fatalf("Case 17: stage must be cancelled after finalization, got %s", finalStage)
	}
