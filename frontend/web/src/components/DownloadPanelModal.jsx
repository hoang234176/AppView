...
export const DownloadPanelModal = ({ isOpen, onClose, tasks = [], onDeleteTask, selectedTab: controlledTab, onSelectedTabChange }) => {
  ...
  const status = (t) => {
    ...
    if (t.stage === 'error') return t.error || 'Tải xuống bị lỗi — có thể tiếp tục tải.';
  ...
  const activeCard = (t) => {
    ...
    return <div key={t.task_id} className="min-h-[108px] space-y-3 rounded-2xl border border-[#383c42] bg-[#202124] p-4">
      ...
      {t.stage === 'password_required' && <div className="flex gap-2"><input value={passwords[t.task_id] || ''} onChange={(e) => setPasswords((old) => ({ ...old, [t.task_id]: e.target.value }))} placeholder="Nhập lại mật khẩu..." className="min-w-0 flex-1 rounded-xl border border-red-500/50 bg-[#1c1d21] px-3 py-1.5 text-xs text-white outline-none" /><button onClick={() => submitPassword(t.task_id)} className="rounded-xl bg-red-600 px-3 text-xs font-bold text-white">Tiếp tục</button></div>}
    </div>;
  };
