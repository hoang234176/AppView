...
- Terminal logging across download lifecycle: added lifecycle terminal logs across Coordinator, Download, and Storage (link inspection, platform detection, resolution, worker assignment, ffmpeg muxing, destination commitment, and final completion). Cookie updates strictly log only updated field names on separate bulleted lines (`  • <field_name>`), never logging secret values or auth tokens.
- YouTube cookie verification accuracy: resolved false-positive verification by testing authentic session state (`LOGGED_IN: true`) on YouTube rather than guest extraction of public videos. Expired or rotated sessions now report invalid status with actionable re-export guidance.
- Storage worker job routing and Meta CDN isolation: fixed issue where Instagram media URLs (`*.fbcdn.net`) were intercepted by `isFacebookRequest` and persisted to `facebook/jobs`. Storage worker now strictly respects `Source` before falling back to URL heuristics, prioritizes Instagram over Facebook, and relocated misplaced Instagram jobs to `instagram/jobs`.
- TikTok IP-block / WAF challenge detection: enhanced error classification so IP block / challenge errors trigger cookie fallback automatically, allowing restricted TikTok posts to resolve cleanly.
- Environment configuration cluster separation: separated `.env` and `.env.example` into dedicated clusters for shared config, Coordinator (deploy), Download (deploy), and Storage (local). Supported `DOWNLOAD_COORDINATOR_WS_URL` and `STORAGE_COORDINATOR_WS_URL` to enable seamless cloud deployment of Coordinator and Download alongside local Storage without configuration collisions.
